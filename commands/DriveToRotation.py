...
class DriveToRotation(Command):
    def __init__(
        ...
        ).with_heading_pid( 9.0, 0, 0.25 )

        self.setName(f"DriveToRotation-{ "FieldOriented" if fieldCentric else "RobotRelative" }")
    ...
    def execute(self):
        ...
        if isinstance(target, Translation2d):
            ...
            if DriverStation.getAlliance() == DriverStation.Alliance.kRed and self.flipRedAlliance:
        ...
        self.drivetrain.set_control(
            self._drive.with_velocity_x(x)
                ...
                .with_max_abs_rotational_rate( self.drivetrain.getMaxAngularRate() )
        )
